...
asset::Mesh asset::LoadModelObj(const char* path, float additional_scale) {
	...
	glm::vec3 mesh_min(std::numeric_limits<float>::infinity());
	glm::vec3 mesh_max(-std::numeric_limits<float>::infinity());
	...
	for (const tinyobj::index_t& index : obj_mesh.indices) {
		...
		if (it == index_mapping.end()) {
			...
			// update mesh bounds
			mesh_min.x = std::min(mesh_min.x, vertex.position.x);
			mesh_min.y = std::min(mesh_min.y, vertex.position.y);
			mesh_min.z = std::min(mesh_min.z, vertex.position.z);
			mesh_max.x = std::max(mesh_max.x, vertex.position.x);
			mesh_max.y = std::max(mesh_max.y, vertex.position.y);
			mesh_max.z = std::max(mesh_max.z, vertex.position.z);
		}
		mesh.indices.push_back(it->second);
	}

	// scale and center the mesh
	glm::vec3 diag = mesh_max - mesh_min;
	float max_size = std::max(diag.x, std::max(diag.y, diag.z));
	float scale = (2.f / max_size) * additional_scale;

	glm::vec3 center = (mesh_min + mesh_max) * 0.5f;
